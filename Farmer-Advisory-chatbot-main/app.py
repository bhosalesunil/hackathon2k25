...
def rule_based_advice(query: str) -> str:
    query = query.lower()
    if ("pest" in query or "disease" in query or "symptom" in query or 
        "blight" in query or "wilt" in query or "rust" in query or "mildew" in query or "spot" in query):
        return (
            "For suspected crop disease: 1) Remove affected leaves, 2) Improve air flow, 3) Use a safe fungicide/pesticide as per label, 4) Monitor daily. "
            "Share crop name, leaf/fruit symptoms, color/shape of spots, and recent weather for precise advice."
        )
...
def get_chatbot_response(query: str) -> dict:
    ...
    try:
        # DB is initialized at startup; we'll save into the main 'queries' table below
        ...
        # Save the query and response to the unified 'queries' table (type='api')
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            save_to_db(query, response, "api")
...
# --- Database setup ---
def init_db():
    DB_PATH = r"E:\Hacthon Project\farmar_chatbot\Farmer-Advisory-chatbot-main\farmer.db"
...
def save_to_csv(query: str, response: str):
    ...
    # CSV file path
    csv_file = os.path.join(data_dir, 'queries.csv')
...
def register():
    if request.method == "GET":
        # Render your registration page template
    ...
    try:
        ...
        land_size = request.form.get("land_size_acres")
        try:
            land_size = float(land_size) if land_size not in (None, "") else None
        except ValueError:
            land_size = None
        preferred_crops = (request.form.get("preferred_crops") or "").strip()

        # Save profile if any profile field was provided
        if any([phone, address, district, state, language, land_size is not None, preferred_crops]):
            c.execute(
                """
                INSERT INTO user_profiles (
                    user_id, phone, address, district, state, language, land_size_acres, preferred_crops
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (user_id, phone, address, district, state, language, land_size, preferred_crops)
            )
...
def login():
    if request.method == 'GET':
        # If a dedicated login template exists, render it; else reuse main.html
        try:
...
def admin_users():
    ...
    c.execute(
        ...
        SELECT u.id, u.name, u.email, u.created_at,
               p.phone, p.address, p.district, p.state, p.language,
               p.land_size_acres, p.preferred_crops
        FROM users u
